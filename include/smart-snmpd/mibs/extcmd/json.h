...
__sm_private int json_parser_init(json_parser *parser, json_config *cfg,
                     json_parser_callback callback, void *userdata);

/** json_parser_free freed memory structure allocated by the parser */
__sm_private int json_parser_free(json_parser *parser);
...
__sm_private int json_parser_string(json_parser *parser, const char *string,
                       uint32_t length, uint32_t *processed);

/** json_parser_char append one single char to the parser
 * return 0 if everything went ok, a JSON_ERROR_* otherwise */
__sm_private int json_parser_char(json_parser *parser, unsigned char next_char);

/** json_parser_is_done return 0 is the parser isn't in a finish state. !0 if it is */
__sm_private int json_parser_is_done(json_parser *parser);

/** json_print_init initialize a printer context. always succeed */
__sm_private int json_print_init(json_printer *printer, json_printer_callback callback, void *userdata);

/** json_print_free free a printer context
 * doesn't do anything now, but in future print_init could allocate memory */
__sm_private int json_print_free(json_printer *printer);

/** json_print_pretty pretty print the passed argument (type/data/length). */
__sm_private int json_print_pretty(json_printer *printer, int type, const char *data, uint32_t length);

/** json_print_raw prints without eye candy the passed argument (type/data/length). */
__sm_private int json_print_raw(json_printer *printer, int type, const char *data, uint32_t length);
...
__sm_private int json_print_args(json_printer *, int (*f)(json_printer *, int, const char *, uint32_t), ...);
...
__sm_private int json_parser_dom_init(json_parser_dom *helper,
                         json_parser_dom_create_structure create_structure,
                         json_parser_dom_create_data create_data,
                         json_parser_dom_append append);
/** free memory allocated by the DOM callback helper */
__sm_private int json_parser_dom_free(json_parser_dom *ctx);

/** helper to parser callback that arrange parsing events into comprehensive JSON data structure */
__sm_private int json_parser_dom_callback(void *userdata, int type, const char *data, uint32_t length);
